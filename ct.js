...
ct.rooms.templates['Room_d2Dqmg'] = {
    name: 'Room_d2Dqmg',
    group: 'ungrouped',
    width: 1920,
    height: 1080,
    objects: JSON.parse('[{"x":464,"y":608,"opacity":1,"tint":16777215,"scale":{"x":0.5,"y":12},"rotation":0,"exts":{},"customProperties":{},"template":"walls"},{"x":1328,"y":608,"opacity":1,"tint":16777215,"scale":{"x":0.5,"y":12},"rotation":0,"exts":{},"customProperties":{},"template":"walls"},{"x":896,"y":976,"opacity":1,"tint":16777215,"scale":{"x":14,"y":0.5},"rotation":0,"exts":{},"customProperties":{},"template":"walls"},{"x":1536,"y":192,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"NextShape"},{"x":896,"y":224,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"CurrentShape"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#AEEAF2',
    
    onStep() {
        /* room Room_d2Dqmg — core_OnActionPress (OnActionPress event) */
...
let shape = this.SpawnShape(this.rand.toString(), ct.pointer.x, this.FLOOR_HEIGHT);

this.rand = this.NextRand;
this.NextRand = Math.round(ct.random.range(1,4));
...
console.log("CURRENT " + this.rand + "NEXT " + this.NextRand);
    ...
    onCreate() {
        /* room Room_d2Dqmg — core_OnRoomStart (On room start event) */
{
this.FLOOR_HEIGHT = 224;
this.X_BOUNDS = {
    L : 512,
    R : 1280
}
...
this.rand = Math.round(ct.random.range(1,4));
this.NextRand = Math.round(ct.random.range(1,4));
console.log("CURRENT " + this.rand + "NEXT " + this.NextRand);
...
ct.matter.on('collisionStart', e => {
    ...
    for (var pair of e.pairs) {
        ...
        if(bodies[0].copy.template == bodies[1].copy.template){
            ...
            var nexTex = bodies[0].copy.template == '6' ? '1' : Number.parseInt(bodies[1].copy.template) + 1;
            console.log(nexTex);
            var pos = {
                x : (pos1.x + pos2.x) / 2 ,
                y : (pos1.y + pos2.y) / 2
            ...
            this.SpawnShape(nexTex.toString(), pos.x, pos.y);
...
this.SpawnShape = (texNum, x, y) =>{
    ...
    let shape = ct.templates.copy(texNum.toString(), Math.min(Math.max(x, this.X_BOUNDS.L), this.X_BOUNDS.R), y);
...
{
this.x = Math.min(Math.max(ct.pointer.x, ct.room.X_BOUNDS.L), ct.room.X_BOUNDS.R)
this.y = ct.room.FLOOR_HEIGHT;
if(ct.room.NextRand != undefined){
    this.tex = ct.room.rand.toString();
}
}
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        textures: {},
        skeletons: {},
        groups: [{"fonts":{"ungrouped":[]},"textures":{"ungrouped":["walls","4","1","2","5","3","6"]},"styles":{"ungrouped":[]},"rooms":{"ungrouped":["Room_d2Dqmg"]},"templates":{"ungrouped":["walls","1","2","3","4","5","6","NextShape","CurrentShape"]},"sounds":{"ungrouped":[]},"emitterTandems":{"ungrouped":[]}}][0],
        /**
